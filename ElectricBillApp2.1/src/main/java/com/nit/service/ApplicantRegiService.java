...
public class ApplicantRegiService {
	...
	public ApplicantRegister loginApplicant(ApplicantRegister appReg)
	{
		appReg.setPassword(app.passwordEncoder().encode(appReg.getPassword()));
		ApplicantRegister applicant = userRepo.findByUserIdAndPassword(appReg.getPassword(), appReg.getUserId());
		return applicant;
	}
